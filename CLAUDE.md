...
- `OCTOWRIGHT_BRIDGE_REQUIRE_TOKEN` — require the `X-Octowright-Token` capability token (held only in the 0600 lockfile) on the leader's `/mcp` transport. **ON by default.** Closes the otherwise-unauthenticated loopback `/mcp` RCE for processes that can't read the lockfile (a different user on a shared host, or a sandbox); does **not** stop a same-user process that can read the lockfile. Set a falsey token (`0`/`off`/`false`/`no`/`never`/`none`/`disabled`) to disable the gate. See **Bridge capability token**.
...
- `OCTOWRIGHT_MCP_SESSION_IDLE_SECONDS` — reap an idle StreamableHTTP MCP session after this many seconds. **OFF by default**, mirroring `OCTOWRIGHT_IDLE_GRACE`'s philosophy: nothing pings the leader to reset a session's idle deadline between real tool calls (only an in-flight call's progress heartbeat does, via `server/_heartbeat.py`), so an ordinary interactive gap — reading output, deciding what to say, watching a slow build/CI run — looks identical to an abandoned session to this timer. Two prior defaults (300s, then 3600s) both reaped live, wanted sessions during normal silence; there is no timeout short enough to catch an abandoned reconnect-storm session without also risking a real one that pauses that long. Set a positive number (e.g. `1800`) to opt in on a shared/CI host that wants bounded memory over long-lived idle sessions; unset/`0`/`off`/`never`/`none`/`disabled` keep it off (the mcp library's own default — it never reaps). When enabled, `http/app.py` sets the timeout on the manager after `streamable_http_app()` builds it (via `_apply_mcp_session_idle_timeout`); the manager resets the deadline on each request, so an ACTIVE session is never reaped — only a truly idle/abandoned one, whose `run_server` task then exits and frees its memory. Without it, an unbounded reconnect-storm (see `OCTOWRIGHT_BRIDGE_MIN_SESSION_SECONDS`) can still leak ~54KB per abandoned session (observed a leader at **2.4GB RSS with zero live browsers** after ~17h; a worse case with heavier concurrent-follower load reached **18.8GB** over ~4.7 days on 2026-07-09) — the flap-guard and split-brain fixes reduce how often such storms happen, but this knob is the direct bound if one still gets through. The parser lives in `http/app.py` (defaults.py at LOC ceiling). **Complementary, unconditional reaper:** `housekeeping._reap_dead_follower_sessions_once` runs every housekeeping cycle regardless of this knob, and reaps by *PID liveness* instead of idle time — bridge-state.json already carries each follower's `(follower_pid, remote_session_id)`, so a follower whose OS process is confirmed gone is terminated immediately, with zero risk of false-positiving on a live client that's merely quiet (the exact risk that keeps idle-time reaping off by default). See `housekeeping.py`'s module docstring (job 3) and `octowright_follower_session_reaped_total` in the metrics table below. **Leader-side storm protection (on by default, version-agnostic):** the idle knob and the pid-liveness reaper both only reclaim sessions whose follower is *gone or quiet* — neither stops a follower that's *alive and storming* (each forwarded RPC opening a fresh session instead of reusing one, the failure mode that put a live leader at **18GB RSS over 2 days** on 2026-07-20). Because every prior storm defense is follower-side (only helps once every client upgrades), the leader now defends itself in `http/mcp_flap_guard.py`, deployable with a single daemon restart: (1) a **per-source new-session rate limit** — a session-creating request (`POST /mcp` with no `Mcp-Session-Id`) beyond `OCTOWRIGHT_MCP_NEW_SESSION_MAX` per `OCTOWRIGHT_MCP_NEW_SESSION_WINDOW_SECONDS` gets `429 + Retry-After`, keyed by the `X-Octowright-Follower` header a current follower sends (old followers share one `anonymous` bucket = the storm, collectively throttled); and (2) a **session-table cap** — housekeeping job 4 evicts the most-idle sessions (abandoned-before-active) whenever the live table exceeds `OCTOWRIGHT_MCP_MAX_SESSIONS`, so memory is bounded no matter how a follower misbehaves. Legit clients create ~1 session and reuse it, so they never approach either limit.
- `OCTOWRIGHT_MCP_MAX_SESSIONS` — cap on concurrent leader `/mcp` sessions before housekeeping LRU-evicts back down to it (the version-agnostic memory bound against a session storm). **Default 256** (on); a real host runs a handful of reused sessions, so this is only reached by pathological accumulation. Eviction sheds sessions silent past the tracker TTL first, a quietly-waiting live session last, and only when over the cap — so it never false-reaps the way an idle timer can. `0`/`off`/`never`/`none`/`disabled`/non-positive disables. Parser + eviction selector in `http/mcp_flap_guard.py`.
- `OCTOWRIGHT_MCP_NEW_SESSION_MAX` / `OCTOWRIGHT_MCP_NEW_SESSION_WINDOW_SECONDS` — per-source sliding-window rate limit on session-creating `/mcp` requests (**defaults 10 per 10s**, on). Over-rate creations get `429 + Retry-After` before reaching the transport, so a storming follower is throttled instead of starving the shared leader. Set `OCTOWRIGHT_MCP_NEW_SESSION_MAX` to a falsey token to disable the limiter. See **Leader-side storm protection** above and `http/mcp_flap_guard.py`.
...
| `octowright_follower_session_reaped_total` | counter | — | Leader MCP sessions terminated by the housekeeping pid-liveness reaper because their follower's OS process was found dead (independent of `OCTOWRIGHT_MCP_SESSION_IDLE_SECONDS`, which reaps by idle time instead). Process-lifetime running total also readable in-process via `octowright_status()["bridge"]["follower_sessions_reaped"]`. |
