...
def _frames_on_disk(session: Any) -> list[dict[str, Any]]:
...
class TestPayloadsReachTheSidecar:
    def test_text_frame_payload_is_persisted(self, tmp_path: Path) -> None:
        ...
        rows = [row for row in _frames_on_disk(session) if row["action"] == "websocket_framesent"]
    ...
    def test_binary_frame_is_persisted_as_base64(self, tmp_path: Path) -> None:
        ...
        rows = [row for row in _frames_on_disk(session) if row["action"] == "websocket_framereceived"]
