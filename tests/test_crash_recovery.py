...
async def test_recover_reload_success_clears_crashed_and_counts() -> None:
    s = _session()
    page = MagicMock()
    page.reload = AsyncMock()
    ok = await crash_recovery._recover(s, page, reload_timeout_ms=15000.0)
    assert ok is True
...
async def test_recover_succeeds_even_if_recorder_marker_fails() -> None:
    ...
    page.reload = AsyncMock()
    ok = await crash_recovery._recover(s, page, reload_timeout_ms=15000.0)
    # Reload succeeded; the best-effort recorder marker failure is swallowed.
    assert ok is True
    assert s._crashed is False
    assert crash_recovery.recovery_stats()["recoveries"] == 1


async def test_recover_reload_failure_counts_and_keeps_crashed() -> None:
    s = _session()
    page = MagicMock()
    page.reload = AsyncMock(side_effect=RuntimeError("still dead"))
    ok = await crash_recovery._recover(s, page, reload_timeout_ms=15000.0)
    assert ok is False
...
def test_schedule_recovery_eligible_creates_tracked_task(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _run() -> None:
        task = crash_recovery.schedule_recovery(s, page)
