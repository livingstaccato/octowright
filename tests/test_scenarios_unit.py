...
class TestLoadYamlScenario:
    ...
    def test_participant_options_round_trip_through_yaml(self, tmp_path: Path) -> None:
        """``options`` is opaque to core -- the loader passes it through
        unmodified for whichever kind owns it, applying no per-kind defaults
        (a plugin kind's own ``resolve_participant`` does that, at launch
        time). Kind-specific coverage (terminal's connector_type defaults,
        SSH field shapes) lives in
        packages/octowright-terminal/tests/test_scenario_adapter.py."""
        path = tmp_path / "opts.yaml"
        path.write_text(
            yaml.safe_dump(
                {
                    "name": "opts",
                    "participants": [
                        {"persona": "a", "kind": "chromium", "role": "player"},
                        {
                            "persona": "b",
                            "kind": "firefox",
                            "role": "monitor",
                            "options": {"custom_flag": True, "nested": {"host": "h", "port": 2222}},
                        },
                    ],
                }
            )
        ...
        s = load_yaml_scenario(path.read_text(), path.stem)
        bare, custom = s.participants
        assert bare.options == {}
        assert custom.options == {"custom_flag": True, "nested": {"host": "h", "port": 2222}}
