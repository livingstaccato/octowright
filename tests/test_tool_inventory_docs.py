...
def test_the_diagram_carries_the_same_counts_and_is_checked(guard) -> None:
    ...
    stale_title = text.replace("(134 tools total", "(126 tools total")
...
def test_a_stale_readme_total_is_reported(guard) -> None:
    ...
    mangled = readme.replace("every core-install tool registers. | 134 |", "every core-install tool registers. | 129 |")
