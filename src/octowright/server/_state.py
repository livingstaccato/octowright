...
mcp = _ProfiledFastMCP(
    ...
    instructions=(
        ...
        "Instead: tell the user Octowright is disconnected and ask them to reconnect it. "
        "For Claude Code (state this confidently): /mcp -> select octowright -> Reconnect. "
        "For any other client, ask which client they are using and have them use its MCP reconnect "
        "control or restart the client. Wait for them to confirm reconnection before resuming."
    ),
)
