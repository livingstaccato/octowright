...
def validate_upload_path(path: str) -> Path:
    ...
    ValueError on rejection (matches the arg-validation style used elsewhere
    in this module, e.g. browser_quick_launch).
    """
...
async def browser_set_input_files(
    ...
    return await pool.get(instance_id).set_input_files(selector, validated)
