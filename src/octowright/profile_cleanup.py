...
def cleanup_stale(stale: list[StaleProfile], dry_run: bool = True) -> dict[str, int]:
    ...
    for sp in stale:
        try:
            ...
            # Also try to clean up the parent persona dir if it's now empty
            # (no engine subdirs left, no profile.yaml). Keep YAMLs intact.
