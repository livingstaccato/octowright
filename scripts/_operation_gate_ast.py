...
def all_function_args(node: ast.FunctionDef | ast.AsyncFunctionDef) -> list[ast.arg]:
...
def seed_param_taint(node: ast.FunctionDef | ast.AsyncFunctionDef, import_map: Mapping[str, str]) -> set[str]:
    tainted: set[str] = set()
    for arg in all_function_args(node):
        ...
        if signal is False:
            ...
            continue
        if arg.arg in SEED_PARAM_NAMES:
            tainted.add(arg.arg)
    return tainted


def seed_session_param_names(node: ast.FunctionDef | ast.AsyncFunctionDef) -> set[str]:
    names: set[str] = set()
    for arg in all_function_args(node):
