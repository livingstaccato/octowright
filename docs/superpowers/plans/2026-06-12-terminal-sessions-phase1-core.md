...
**Phase 2 — carry-overs from the Phase 1 final review (must address):**
- **macOS EOF self-detection gap.** The engine's `terminal_stop("eof")` fires only when the connector's `is_connected()` flips False, which on macOS never happens for an exited child (a post-exit PTY master read returns `b""`, not `EIO`, so the connector keeps `_connected=True`). On Linux it works. The poll loop doesn't leak (explicit `stop()` cancels it), but spec §9's "connector EOF → self-evicts + records `terminal_stop`" is unmet on macOS, and Phase 2 external-close eviction depends on it. Wire a child-exit watcher (e.g. `os.waitpid`-based liveness, or detect `b""` EOF) rather than relying solely on `is_connected()`. The Phase 1 test tolerates `reason in {"eof","closed"}` — tighten it once fixed.
- **Cleared-buffer delta mis-record (latent).** `translate.py:_delta` can't distinguish a 32KB rotation from a `clear()`-then-new-output transition; a coincidental suffix/prefix overlap would drop a leading char. Not reachable in Phase 1 (the engine never calls the connector's `clear()`, and ANSI clear-screen sequences accumulate as raw bytes in the prefix-growing buffer). When a `clear`/`reset` control is wired, record a `terminal_clear` boundary action that resets `_last_screen=""`, or key the delta on the connector's `screen_hash`/a sequence number.
