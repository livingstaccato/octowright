...
Current totals (verified against `selftest` 2026-06-29 at 125 tools on a core install): **94** profile-scoped + **7** always-on + **24** all-only = **125 total**.

With the optional `octowright[terminal]` extra installed, the `terminals`
profile adds 7 more tools for **132 total**.
...
| `advanced` | 30 | Inspection, assertions, ARIA locators, summaries, viewport sync, capture cache, export script, relaunch. |
...
| _(all-only)_ | 24 | Registers only when **no** `--profile` filter is active. Frames/tabs/popups, network mocking, dialog policy, traces, roster fan-out, macro-artifact extras, cleanup. |
...
`capture_cleanup`, `capture_create`, `capture_get`, `capture_lines`, `capture_list`, `capture_search`, `capture_summary`, `browser_console_messages`, `browser_console_summary`, `browser_downloads`, `browser_downloads_summary`, `browser_each`, `browser_evaluate`, `browser_expect_js`, `browser_expect_selector`, `browser_expect_text`, `browser_expect_url`, `browser_export_script`, `browser_get_text_by`, `browser_network_requests`, `browser_network_summary`, `browser_observe`, `browser_relaunch_fluid`, `browser_resize`, `browser_recording_path`, `browser_snapshot`, `browser_tail_recording`, `browser_wait_for_download`, `browser_viewport_status`, `browser_viewport_sync`
...
### All-only (24) — register only when no `--profile` filter is active

`browser_capture_and_close`, `browser_drag`, `browser_hover`, `browser_list_frames`, `browser_mock_route`, `browser_navigate_back`, `browser_open_trace`, `browser_open_url`, `browser_reset_frame`, `browser_select_option`, `browser_set_dialog_policy`, `browser_set_input_files`, `browser_spawn_roster`, `browser_switch_frame`, `browser_unmock_route`, `macro_artifact_critical_points_get`, `macro_artifact_critical_points_set`, `macro_artifact_status`, `macro_artifact_verify`, `page_close`, `page_list`, `page_switch`, `recordings_cleanup`, `run_test_suite`
