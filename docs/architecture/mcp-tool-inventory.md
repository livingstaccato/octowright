...
Current totals (last verified against `selftest` at 106 tools): **71** profile-scoped + **7** always-on + **28** all-only = **106 total**.
...
|---|---:|---|
| `core` | 15 | Minimum to drive a browser. |
| `advanced` | 22 | Inspection, assertions, ARIA locators, viewport sync, capture cache, export script, relaunch. |
| `macros` | 9 | Record → save → replay pipeline. |
...
| _(all-only)_ | 28 | Registers only when **no** `--profile` filter is active. Frames/tabs/popups, network mocking, dialog policy, downloads, traces, per-roster fan-out helpers, cleanup. |

## Per-profile tool list

### `core` (15)

`browser_brief`, `browser_click`, `browser_close`, `browser_close_all`, `browser_fill`, `browser_launch`, `browser_list`, `browser_navigate`, `browser_press_key`, `browser_quick_launch`, `browser_read_markdown`, `browser_screenshot`, `browser_suggest_for_url`, `browser_type`, `browser_wait_for`

### `advanced` (22)

`browser_click_by`, `browser_console_messages`, `browser_evaluate`, `browser_expect_js`, `browser_expect_selector`, `browser_expect_text`, `browser_expect_url`, `browser_export_script`, `browser_fill_by`, `browser_get_text_by`, `browser_recording_path`, `browser_relaunch_fluid`, `browser_resize`, `browser_snapshot`, `browser_tail_recording`, `browser_viewport_status`, `browser_viewport_sync`, `capture_cleanup`, `capture_create`, `capture_get`, `capture_list`, `capture_search`

### `macros` (9)

`macro_compile`, `macro_delete`, `macro_explain`, `macro_lint`, `macro_list`, `macro_repair_preview`, `macro_run`, `macro_run_sequence`, `macro_save`
