...
Current totals (verified against `selftest` 2026-08-15 at 126 tools on a core install): **95** profile-scoped + **7** always-on + **24** all-only = **126 total**.

With the optional `octowright[terminal]` extra installed, the `terminals`
profile adds 7 more tools for **133 total**.
