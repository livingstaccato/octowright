...
- `OCTOWRIGHT_MCP_SESSION_IDLE_SECONDS` — reap an idle StreamableHTTP MCP session after this many seconds (default `300`; `0`/`off`/`never`/`none`/`disabled` opts out). The mcp session manager defaults `session_idle_timeout=None` — it **never** reaps sessions, so every session's per-session server task + transport lingers in the manager's task group even after the client vanishes. A reconnect storm (see `OCTOWRIGHT_BRIDGE_MIN_SESSION_SECONDS`) therefore leaked ~54KB per abandoned session, unbounded — observed a leader at **2.4GB RSS with zero live browsers** after ~17h. `http/app.py` sets the timeout on the manager after `streamable_http_app()` builds it (via `_apply_mcp_session_idle_timeout`); the manager resets the deadline on each request, so an ACTIVE session is never reaped — only a truly idle/abandoned one, whose `run_server` task then exits and frees its memory. The parser lives in `http/app.py` (defaults.py at LOC ceiling).
