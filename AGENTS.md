...
**Bridge capability token**: the loopback `/mcp` transport drives browsers (RCE-equivalent) and otherwise has **no auth** — any local process can POST to it. The leader generates a random token, writes it to the 0600 lockfile (`LeaderInfo.token`, via `cli/serve`), and requires it on `/mcp` (`http/bridge_auth.BridgeTokenGuard`, wrapped *inside* the host/origin `SensitiveASGIGuard`); the follower reads it back (`proxy_runtime.resolve_leader_token`, gated by the same loopback check as `resolve_leader_url`) and presents it as the `X-Octowright-Token` header. A process that **can't read the lockfile** — a *different user* on a shared host, or a *sandboxed* process — therefore can't drive the leader. **Limits (be honest):** this does NOT defend against a *same-user* process that reads the 0600 lockfile (it gets the token; the lockfile is the same-user trust boundary), and does NOT close the lockfile-poisoning MITM (an attacker who rewrites the lock writes the token too). On by default; disable with `OCTOWRIGHT_BRIDGE_REQUIRE_TOKEN` set to a falsey token. An inline (`--no-singleton`) leader uses an empty token (gate off) since it has no lockfile. The **same** token also gates the follower-only `GET /api/mcp-events` SSE channel (`http/routes/mcp_events._require_token`) — it carries the same follower→leader trust as `/mcp` (a different-user/sandboxed process could otherwise subscribe to the leader's crash/close/driver notification stream), and the browser dashboard never calls it, so the gate is safe on by default. **Browser dashboard (opt-in pairing):** the browser-facing surface (`/api/sessions`, media, `/api/dashboard/events`, `/tail`+screencast WS, persona/scenario/macro writes) can additionally be gated by **dashboard pairing** (`OCTOWRIGHT_DASHBOARD_REQUIRE_PAIRING`, OFF by default). Embedding the token in the served page would leak it to any loopback fetcher, so the token reaches the *human* out-of-band instead: `octowright dashboard` (same-user, reads the 0600 lockfile) POSTs `/api/pair/mint` with the token, gets a single-use 60s ticket, and prints `http://127.0.0.1:PORT/pair#<ticket>` to the tty (ticket in the URL *fragment* — never sent on navigation, never logged); the `/pair` page redeems it for an HttpOnly `SameSite=Strict` session cookie. Guarded routes then accept either that cookie (browser) or the `X-Octowright-Token` header (follower/programmatic) — enforced inside `http/exposure.guard_sensitive_http` (opt-out per route via `pairing_exempt`, used only by the pairing bootstrap routes and `/new-tab`, which launched browsers must reach cookieless) and mirrored at both WS handshakes. State is in-memory (`http/pairing.py`: single-use tickets, bounded session LRU) so a leader restart invalidates everything. Same-user processes are *not* defended (they read the lockfile and mint their own ticket) — the same honest limit as the /mcp token. `--open` avoids putting the ticket in browser argv (world-readable `/proc/PID/cmdline`) by opening a 0700-dir `file://` redirect page instead.
...
- `OCTOWRIGHT_DASHBOARD_REQUIRE_PAIRING` — opt-in dashboard pairing gate for the browser-facing surface (sessions/media/events/tail/screencast/writes). **OFF by default** (type-the-URL UX preserved). Set a truthy token (`1`/`on`/`true`/`yes`) to require a paired session cookie (mint one with `octowright dashboard`) or the `X-Octowright-Token` header on every guarded route. See **Bridge capability token → Browser dashboard (opt-in pairing)**.
